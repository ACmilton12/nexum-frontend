...
export default function PdfTemplateModal({ isOpen, onClose, portfolioId }: PdfTemplateModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col md:flex-row shadow-2xl animate-in zoom-in-95 duration-200">

        {/* Left Side: Options */}
        <div className="w-full md:w-1/2 p-6 md:p-8 flex flex-col bg-slate-50 border-r border-slate-200">
          <div className="flex justify-between items-center mb-6 md:hidden">
            <h2 className="text-xl font-bold text-slate-900">{t('print.modal_title')}</h2>
            <button onClick={onClose} className="p-2 text-slate-400 hover:text-slate-600 rounded-full hover:bg-slate-100 transition-colors">
              <X size={20} />
            </button>
          </div>

          <h2 className="hidden md:block text-2xl font-bold text-slate-900 mb-2">{t('print.modal_title')}</h2>
          <p className="text-slate-500 mb-8 text-sm">{t('print.modal_desc')}</p>
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-4">
            {templates.map((tpl) => (
              ...
              >
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5 ${selectedTemplate === tpl.id ? 'border-[#003087] bg-[#003087]' : 'border-slate-300'
                  }`}>
                  {selectedTemplate === tpl.id && <Check size={12} className="text-white" />}
                </div>
                <div>
                  <h3 className={`font-bold text-base mb-1 ${selectedTemplate === tpl.id ? 'text-[#003087]' : 'text-slate-800'}`}>
                    {tpl.name}
                  </h3>
          ...
          <div className="mt-8 flex gap-3 pt-4 border-t border-slate-200">
            <button
              onClick={onClose}
              className="flex-1 py-3 px-4 rounded-xl text-slate-600 font-bold bg-white border border-slate-200 hover:bg-slate-50 transition-colors"
            >
              {t('print.cancel')}
            </button>
            <button
              onClick={handleGenerate}
              className="flex-1 py-3 px-4 rounded-xl text-white font-bold bg-[#003087] hover:bg-blue-800 shadow-lg shadow-blue-900/20 transition-all hover:-translate-y-0.5 active:translate-y-0 flex items-center justify-center gap-2"
            >
              <FileText size={18} />
              {t('print.download')}
            </button>
        ...
        {/* Right Side: Preview */}
        <div className="hidden md:flex w-full md:w-1/2 bg-slate-200/50 p-8 flex-col items-center justify-center relative">
          ...
          <div className="w-full max-w-[300px] aspect-[1/1.414] bg-white shadow-2xl rounded-lg p-2 transition-all duration-500 transform hover:scale-105">
