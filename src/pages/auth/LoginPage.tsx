...
const LoginCarousel = () => {
  ...
  const slides = [
    {
      ...
      desc: 'Conecta con oportunidades y muestra tu talento al mundo'
    },
    {
      img: prueba09,
      title: 'Proyectos que hablan por ti',
      desc: 'Sube tu trabajo y déjalo brillar ante los empleadores'
    },
    {
      img: prueba10,
      title: 'Verificado por UMSS · FCyT',
      desc: 'Tu perfil con respaldo académico real'
    }
  ]
  ...
  return (
    <div className="flex flex-col items-center w-full">
      ...
      />
      <h2 className="text-xl lg:text-2xl font-bold text-center mb-2">{slides[idx].title}</h2>
      <p className="text-sm text-center opacity-80 mb-6">{slides[idx].desc}</p>
      {/* Dots */}
...
const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let rawMessage =
        error?.message || 'Credenciales inválidas. Verifica tus datos e inténtalo nuevamente.'
          ...
          <div className="w-full md:w-1/2 flex flex-col rounded-2xl items-center justify-center p-6 sm:p-8 md:p-10 bg-background">
            ...
            <div className="w-full bg-white rounded-2xl border border-gray-200 p-5 sm:p-6 shadow-md">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {/* Contraseña */}
                <div>
                  <label className="text-sm font-medium text-textMain block mb-1">Contraseña</label>
                  <div className="flex items-center border border-gray-300 rounded px-3 py-2 gap-2 focus-within:border-primary">
