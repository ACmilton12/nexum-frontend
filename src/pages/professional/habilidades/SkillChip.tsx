...
export default function SkillChip({ skill, onEdit, onToggleDisable, toggling }: {
  ...
  return (
    <div className={`flex items-center gap-2 bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-lg px-2.5 sm:px-3 py-2 transition-colors max-w-full ${toggling ? "opacity-50 border-gray-200" :
      isPending ? "border-amber-300 bg-amber-50/60" :
        isDisabled ? "border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/40 opacity-60" :
      ...
      <span className={`text-sm font-medium ${isDisabled ? "text-gray-400 dark:text-gray-500 line-through decoration-gray-300 dark:decoration-gray-700" : "text-gray-800 dark:text-white"}`}>
        {skill.nombre}
      </span>
      ...
      <button
        ...
        className={`ml-1 p-0.5 rounded transition-colors disabled:cursor-not-allowed ${isPending || isDisabled ? "text-gray-300 dark:text-gray-700" : "text-gray-400 dark:text-gray-500 hover:text-primary dark:hover:text-blue-400"
          }`}>
        <IconEdit />
      </button>
      ...
      <button
        ...
        className={`p-0.5 rounded transition-colors disabled:cursor-not-allowed ${isPending ? "text-gray-300 dark:text-gray-700" :
          isDisabled ? "text-gray-400 dark:text-gray-500 hover:text-primary dark:hover:text-blue-400" :
