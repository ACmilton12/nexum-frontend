    ...
    <div className="flex flex-col gap-5 w-full max-w-[520px]">
      ...
      <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-bold text-[#1a1a2e] dark:text-gray-300">Título del proyecto</label>
            <input
              type="text"
              value={title}
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] font-bold text-[#1a1a2e] dark:text-gray-300">Categoría</label>
            <select
              value={categoryId}
            ...
            >
              ...
              <option value="otros" className="dark:bg-slate-800">Otros</option>
            </select>
            <p className="text-[11px] text-[#5b6472] dark:text-gray-500 mt-0.5 leading-relaxed">
        ...
        <div className="flex flex-col gap-1.5 -mt-2">
          <label className="text-[13px] font-bold text-[#1a1a2e] dark:text-gray-300">Descripción</label>
          <textarea
            rows={4}
            value={description}
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-[13px] font-bold text-[#1a1a2e] dark:text-gray-300">Enlace del proyecto</label>
          <input
            type="text"
            value={projectUrl}
            onChange={(e) => {
            ...
            className={`w-full h-10 px-3 text-sm bg-white dark:bg-slate-800 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0030871a] transition-all text-[#1a1a2e] dark:text-white placeholder:text-gray-400 dark:placeholder:text-gray-600 ${
              projectUrlError ? "border-red-400 focus:border-red-400 focus:ring-red-100" : "border-gray-200 dark:border-gray-700 focus:border-[#003087]"
            }`}
        ...
        <div className="flex flex-col gap-1.5 mt-1" ref={dropdownRef}>
          <label className="text-[13px] font-bold text-[#1a1a2e] dark:text-gray-300">Tecnologías</label>
          <div className="relative">
            <div
              className={`min-h-10 w-full px-3 py-2 text-sm bg-white dark:bg-slate-800 border rounded-lg transition-all cursor-pointer flex items-center justify-between gap-2 ${
                isTechDropdownOpen ? "ring-2 ring-[#0030871a] border-[#003087]" : "border-gray-200 dark:border-gray-700"
              }`}
              onClick={() => setIsTechDropdownOpen(!isTechDropdownOpen)}
            >
              <div className="flex flex-wrap gap-1.5 flex-1">
                {selectedSkills.length === 0 ? (
                  <span className="text-[#5b6472] dark:text-gray-600 select-none">Ej. React, Python...</span>
                ) : (
                  selectedSkills.map((skill) => (
                    <span key={skill.id} className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-[#0030871a] dark:bg-blue-900/30 text-[#003087] dark:text-blue-300 text-[11px] font-semibold">
                      {skill.name}
              ...
              <ChevronDown size={16} className={`text-[#5b6472] dark:text-gray-500 transition-transform flex-shrink-0 ${isTechDropdownOpen ? "rotate-180" : ""}`} />
            </div>
            {isTechDropdownOpen && (
              <div className="absolute z-20 w-full mt-1 bg-white dark:bg-slate-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl max-h-48 overflow-y-auto py-1">
                {availableSkills.length === 0 && <div className="px-3 py-2 text-sm text-gray-400 dark:text-gray-600">Cargando habilidades...</div>}
                {availableSkills.map((skill) => {
                  ...
                  return (
                    ...
                    >
                      <span className={isSelected ? "font-bold text-[#003087] dark:text-blue-400" : "text-[#1a1a2e] dark:text-gray-300"}>
                        {skill.name} <span className="text-[11px] text-gray-400 dark:text-gray-600 ml-1 font-normal">({skill.category})</span>
                      </span>
                      {isSelected && <Check size={16} className="text-[#003087] dark:text-blue-400" />}
                    </div>
                  );
                })}
              </div>
        ...
        <div className="flex flex-col-reverse sm:flex-row justify-between sm:items-center gap-3 pt-6 mt-2 border-t border-gray-100 dark:border-gray-800">
          <button
            type="button"
            onClick={onCancel}
      ...
      {showSuggestModal && (
        <div className="fixed inset-0 z-[1050] flex items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-slate-900 rounded-xl shadow-2xl p-6 w-full max-w-[400px] mx-4 flex flex-col gap-4 border border-gray-100 dark:border-gray-800" onClick={(e) => e.stopPropagation()}>
            <button
            ...
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-[16px] font-bold text-[#1a1a2e] dark:text-white">Sugerir nueva categoría</h3>
            </div>
            
            <p className="text-[13px] text-[#5b6472] dark:text-gray-400 leading-relaxed mb-1">
              Si no encuentras una categoría adecuada, puedes sugerir una nueva. Un administrador la revisará.
            </p>
 
            <div className="flex flex-col gap-3">
              <div className="flex flex-col gap-1.5">
                <label className="text-[13px] font-bold text-[#1a1a2e] dark:text-gray-300">Nombre de la categoría</label>
                <input
                  type="text"
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[13px] font-bold text-[#1a1a2e] dark:text-gray-300">Justificación</label>
                <textarea
                  rows={3}
